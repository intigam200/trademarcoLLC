...
function UsersPanel() {
  ...
  return (
    <AdminCard title="Users">
      {error && (
        <div style={{ fontSize: 12, color: ADMIN_COLORS.danger, marginBottom: 12 }}>Could not load users: {error}</div>
      )}
      {users === null && !error && (
        <div style={{ fontSize: 13, color: ADMIN_COLORS.medGray }}>Loading&hellip;</div>
      )}
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {(users ?? []).map((u) => (
          <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", border: `1px solid ${ADMIN_COLORS.border}`, borderRadius: 8 }}>
            <span style={{ width: 36, height: 36, borderRadius: "50%", background: ADMIN_COLORS.iconBlueBg, color: ADMIN_COLORS.iconBlue, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, flexShrink: 0 }}>
              {(u.email || "A").charAt(0).toUpperCase()}
            </span>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: ADMIN_COLORS.navy }}>
                {u.email} {u.email === email && <span style={{ color: ADMIN_COLORS.medGray, fontWeight: 400 }}>(you)</span>}
              </div>
              <div style={{ fontSize: 12, color: ADMIN_COLORS.medGray, textTransform: "capitalize" }}>{u.role}</div>
            </div>
          </div>
        ))}
      </div>
      <ComingSoonNote>
        Every account here was created directly in Supabase Auth — there is no public registration route. Role management
        (admin vs. staff permissions) beyond the current single &ldquo;admin&rdquo; role is planned for a later phase.
      </ComingSoonNote>
    </AdminCard>
  );
}
...
function SecurityPanel() {
  const rows = [
    { label: "Authentication", value: "Supabase Auth — email + password" },
    { label: "Session", value: "Managed by Supabase Auth (JWT, auto-refreshed)" },
    { label: "Public sign-up", value: "Disabled — no registration route exists in this app" },
    { label: "Password reset", value: "Via Supabase Auth (Dashboard, or a reset-password flow if added later)" },
    { label: "Row Level Security", value: "Enabled on every table — anon reads active/published rows only" },
  ];
  return (
